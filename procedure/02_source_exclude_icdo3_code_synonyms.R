...
for (i in 1:length(input_vector)) {
        ...
        if (!is.logical(input_vector[[i]]) && !(input_vector[[i]] %in% c(NA, "NA"))) {
                        ...
                        if (is.na(input_fact_concept[i])) {
                                ...
                                output[[i]] <-
                                        chariot::query_phrase_synonym(input_phrase, type = type) %>%
                                        filter_for_settings() %>%
...
for (i in 1:length(output)) {
        ...
        if (!is.logical(output[[i]])) {
                ...
                if (type %in% c("exact")) {
                        new_col_name <- "Source Remove ICDO3 Synonym Exact"
                } else {
                        new_col_name <- "Source Remove ICDO3 Synonym Like"
                }
        ...
        while (length(types) > 0) {
                ...
                for (i in 1:length(output)) {
                        ...
                        if (!is.logical(output[[i]])) {
                                ...
                                if (type %in% c("exact")) {
                                        new_col_name <- "Source Remove ICDO3 Code Syn Exact"
                                } else {
                                        new_col_name <- "Source Remove ICDO3 Code Syn Like"
                                }
