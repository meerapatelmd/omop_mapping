# Exact and Like style searches for phrases found after the first colon in the string. For example an input of "Meera: Patel" would search "Patel".
...
for (i in 1:length(input_vector)) {
        ...
        if (!is.logical(input_vector[[i]]) && !(input_vector[[i]] %in% c(NA, "NA"))) {
                        ...
                        if (is.na(input_fact_concept[i])) {
                                        ...
                                        first_word <- stringr::str_remove_all(first_word, "[']{1}|[?]{1}$")

                                        first_word <- trimws(first_word, "both")
...
for (i in 1:length(output)) {
        ...
        if (!is.logical(output[[i]])) {
                ...
                if (type %in% c("exact")) {
                        new_col_name <- "Source After Colon Exact"
                } else {
                        new_col_name <- "Source After Colon Like"
                }
