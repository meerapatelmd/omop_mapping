...
target_col <- term_col
input2 <- read_workfile(routine = "03_search_terms")
...
while (length(types) > 0) {
        ...
        if (type == "exact") {
                ...
                new_col_name <- "Search Term Exact"

        } else {

                new_col_name <- "Search Term Like"
        }
                ...
                if (!is.logical(input_vector[[i]])) {
                                ...
                                if (is.na(input_fact_concept[i])) {
                                        ...
                                        for (j in 1:length(input_vector[[i]])) {
                                                output[[i]] <- list()
                                                output[[i]][[j]] <-
                                                        chariot::query_phrase(phrase = input_vector[[i]][j], type = type) %>%
