if (interactive()) {
                ...
                new_col_name <- "Source Each Word UptoDate"
                                                                ...
                                                                if (nchar(input_concept) > source_skip_nchar) {
                                                                        ...
                                                                        if (length(Words)) {
                                                                                ...
                                                                                for (j in 1:length(Words)) {
                                                                                        ...
                                                                                        word_resultset <-
                                                                                                search_drug_in_uptodate(drug = Words[j])

                                                                                        #if (length(word_resultset) > 0) {

                                                                                        output[[i]][[j]] <-
                                                                                                word_resultset

                                                                                        names(output[[i]])[j] <- Words[j]
                                                                                        #}
                                                                                ...
                                                                                if (n_words > 0) {
                                                                                        ...
                                                                                        output[[i]] <-
                                                                                                output[[i]] %>%
                                                                                                dplyr::select(Word, contains("Brand Name")) %>%
                                                                                                tidyr::pivot_longer(cols = contains("Brand Name"),
                                                                                                                    names_to = "Synonym Type",
                                                                                                                    values_to = "Synonym",
                                                                                                                    values_drop_na = TRUE) %>%
                                                                                                dplyr::select(Word, Synonym) %>%
                                                                                                dplyr::distinct() %>%
                                                                                                rubix::split_deselect(column_name = "Word") %>%
                                                                                                rubix::map_names_set(unlist) %>%
                                                                                                purrr::map(cave::vector_to_string) %>%
                                                                                                purrr::map(rubix::vector_to_tibble, new_col = UpToDate) %>%
                                                                                                dplyr::bind_rows(.id = "Word") %>%
                                                                                                dplyr::mutate(UpToDate = paste0(Word, ": ", UpToDate)) %>%
                                                                                                dplyr::select(!!new_col_name := UpToDate)
                                                                                        } else {
                                                                                                output[[i]] <- NA
                                                                                                output[[i]] <-
                                                                                                        output[[i]] %>%
