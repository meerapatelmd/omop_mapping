...
# Search Settings
types <- c("exact") # Can be either or both of c("exact", "like"), but "like" is not advised because the return can be a massive number of rows
        ...
        if (!is.logical(input_vector[[i]]) && !(input_vector[[i]] %in% c(NA, "NA"))) {
                        ...
                        if (is.na(input_fact_concept[i])) {
                                ...
                                if (grepl("[(]{1}", input_vector[i])) {
                                        ...
                                        print(first_word)
                                output[[i]] <-
                                        chariot::query_phrase_synonym(first_word, type = type) %>%
                                        filter_for_settings() %>%
...
for (i in 1:length(output)) {
        ...
        if (!is.logical(output[[i]])) {
                ...
                if (type %in% c("exact")) {
                        new_col_name <- "Source No Parentheses Exact Synonym"
                } else {
                        new_col_name <- "Source No Parentheses Like Synonym"
                }
