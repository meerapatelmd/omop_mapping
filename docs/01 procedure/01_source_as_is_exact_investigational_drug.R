...
new_col_name <- "Source Investigational Drug Exact"
...
for (i in 1:nrow(input3)) {
                        ...
                        if (nchar(input_concept) > source_skip_nchar) {
                                ...
                                if (grepl("^[A-Za-z]{1,}[-| ]{0,1}[0-9]{1,}$", input_concept)) {

                                                native_input_concept <- input_concept
                                                input_concept_no_dash <- stringr::str_remove_all(input_concept, "[-]{1}")
                                                input_concept_dash <- stringr::str_replace_all(input_concept_no_dash,
                                                                                               "([A-Za-z]{1,})([0-9]{1,}$)",
                                                                                               "\\1-\\2")


                                                native <-
                                                        query_phrase_in_athena(phrase = native_input_concept,
                                                                               type = type,
                                                                               synonym = TRUE) %>%
                                                ...
                                                if (nrow(native) == 0) {
                                                                ...
                                                                no_dash <-
                                                                        query_phrase_in_athena(phrase = input_concept_no_dash,
                                                                                               type = type,
                                                                                               synonym = TRUE) %>%
                                                                        chariot::merge_concepts(into = `Concept`) %>%
                                                                        dplyr::select(!!new_col_name := `Concept`)
                                                                ...
                                                                if (nrow(no_dash)) {
                                                                        ...
                                                                        if (nrow(dash) == 0) {
                                                                                ...
                                                                                synonyms <- synonyms_from_chemidplus(
                                                                                                        stringr::str_replace_all(native_input_concept, "[ ]{1}", "%20"))
                                                                                synonyms <-
                                                                                        grep("[0-9]|\\[", synonyms, value = TRUE, invert = TRUE)



                                                                                dash <-
                                                                                        tibble(!!new_col_name := paste0("Not Found: ",
