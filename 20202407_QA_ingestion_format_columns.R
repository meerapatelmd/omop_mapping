...
dd <- broca::read_full_excel("~/Memorial Sloan Kettering Cancer Center/KM COVID - General/COVIDScienceDatabase_DataDictionary - KM Master Dictionary.xlsx")

dd <- dd$`DDict Master Metadata` %>%
        cartograph::standardize_dd_cols() %>%
        dplyr::select(all_of(cartograph::list_std_colnames()))
