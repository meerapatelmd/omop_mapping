if (interactive()) {
                ...
                new_col_name <- "Source Each Word Exact Synonym"
                                                ...
                                                if (nchar(input_concept) > source_skip_nchar) {
                                                        ...
                                                        if (length(Words)) {
                                                                ...
                                                                for (j in 1:length(Words)) {
                                                                        ...
                                                                        sql_statement <- paste0("SELECT c.*, s.concept_synonym_name
                                                                                                FROM concept c
                                                                                                INNER JOIN concept_synonym s
                                                                                                ON s.concept_id = c.concept_id
                                                                                                WHERE c.vocabulary_id = 'HemOnc'
                                                                                                        AND LOWER(s.concept_synonym_name) = '", Words[j], "';")
                                                                        word_resultset <-
                                                                                police::try_catch_error_as_null(chariot::query_athena(sql_statement = sql_statement))
