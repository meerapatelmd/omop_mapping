if (interactive()) {
                ...
                new_col_name <- "Source Each Word Cancer.gov"
                                                                ...
                                                                if (nchar(input_concept) > source_skip_nchar) {
                                                                        ...
                                                                        if (length(Words)) {
                                                                                ...
                                                                                for (j in 1:length(Words)) {
                                                                                        ...
                                                                                        word_resultset <-
                                                                                                search_cancer.gov_dictionary(phrase = Words[j])
                                                                                ...
                                                                                if (n_words > 0) {
                                                                                        ...
                                                                                        output[[i]] <-
                                                                                                output[[i]] %>%
                                                                                                dplyr::select(DRUG, Synonym) %>%
                                                                                                dplyr::distinct() %>%
                                                                                                rubix::split_deselect(column_name = "DRUG") %>%
                                                                                                rubix::map_names_set(unlist) %>%
                                                                                                purrr::map(cave::vector_to_string) %>%
                                                                                                purrr::map(rubix::vector_to_tibble, new_col = CancerGov) %>%
                                                                                                dplyr::bind_rows(.id = "DRUG") %>%
                                                                                                dplyr::mutate(CancerGov = paste0(DRUG, ": ", CancerGov)) %>%
                                                                                                dplyr::select(!!new_col_name := CancerGov) %>%
                                                                                                filter_max_250()
                                                                                        } else {
                                                                                                output[[i]] <- NA
                                                                                                output[[i]] <-
